...
- **1 — Weighted camera mosaic.** Focus mode gives one camera a larger tile while fitting the remaining cameras around it. The primary keeps its native aspect, while uniform 16:9 child panes reflow beside and below it using their own saved Live View framing. Use **Make primary** on a tile to change the focus; the selection survives reloads. Automatic and Custom layouts remain available. Custom supports direct move and resize controls. Focus and Custom use the desktop layout; phones retain the existing primary-camera layout.
...
To reproduce a tall primary among thirteen cameras, start with `PREVIEW_CAMERA_COUNT=13 PREVIEW_PRIMARY_ASPECT=0.75 npm run preview:workspace`, then make Driveway primary. Use `1.333333` for a landscape 4:3 primary.
