...
DETECTOR_CAPTURE_FIELDS = frozenset({"live_sample_fps", "native_live_detection"})
...
def manager_owned_config(config: AppConfig) -> dict:
    ...
    payload["gstreamer_capture"] = {
        "detector": {name: getattr(config.detector, name) for name in (
            "enabled", "backend", "native_live_detection", "model_path", "model_xml", "labels_path", "labels", "device", "nms_threshold",
        )},
