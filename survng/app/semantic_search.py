...
class SemanticSearchService(DisabledSemanticSearch):
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            ...
            # Admission retries retain ownership in the queue. Other failures
            # release it so durable projection reconciliation can submit again.
            if isinstance(event.get("_semantic_revision"), _SemanticEventRevision):
                event["_semantic_revision"].pending = False
