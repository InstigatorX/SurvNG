...
class EventStore(
        ...
        with self._lock, self._connect() as conn:
            ...
            if portable_snapshot and portable_snapshot != replaced_snapshot:
                ...
                if previous_area and replacement_area and replacement_area < previous_area:
                    # A replay is allowed to add diagnostic evidence, but cannot
                    # install smaller pixels over an already improved cover.
                    self._archive_observations(conn, event_id, replacement, portable_snapshot)
                    return None
            preserved = [
                item
                for item in existing
            ...
            updated = self._finish_evidence_commit(conn, event_id, row, reason="evidence_refined", cover_satisfied=bool(portable_snapshot and any(item.get("label") and item.get("frame_source") in {"recorded_main", "buffered_main"} and item.get("snapshot_visible") is not False and item.get("incident_eligible") is not False for item in replacement)))
        ...
        with self._lock, self._connect() as conn:
            ...
            if row is not None:
                ...
                for item in objects:
                    ...
                    matched += 1
                if matched > 0:
                    self._archive_observations(conn, event_id, list(candidates.values()), portable_snapshot)
                    replaced_snapshot = str(row["snapshot_path"] or "")
                    conn.execute(
                        """
                        update events
    ...
    def promote_refinement_cover(
        ...
        *,
        ...
        if source == "buffered_main":
            # An exact encoded access-unit reference is not an exact UTC clock.
        ...
        with self._lock, self._connect() as conn:
            ...
            self._archive_observations(conn, event_id, candidates, portable_snapshot)
            for item in objects:
                if not isinstance(item, dict) or not item.get("label"):
                    continue
            ...
            updated = self._finish_evidence_commit(conn, event_id, row, reason="recorded_cover", cover_satisfied=(source in {"recorded_main", "buffered_main"}))
        if replaced_snapshot and replaced_snapshot != portable_snapshot:
            self._delete_snapshot_if_unreferenced(replaced_snapshot)
        if diagnostics is not None:
