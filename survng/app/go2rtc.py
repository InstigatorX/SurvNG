...
class Go2RtcAdapter:
    ...
    def snapshot(self, camera: CameraConfig, source: str = "live") -> bytes:
        ...
        if any(codec in {"H265", "HEVC"} for codec in codecs):
    ...
    def stream_info(self, camera: CameraConfig, source: str = "live") -> dict[str, Any]:
        ...
        return {
            ...
            "video_codec": codec,
            "video_codecs": codecs,
            "compatibility": "h264" if codec in {"H265", "HEVC"} else "native",
        }
    ...
    def websocket_url(self, camera: CameraConfig, source: str = "live", compatibility: str = "native") -> str:
        ...
        return f"ws://{stream.host}:{self.api_port}/api/ws?src={quote(stream_name, safe='')}"
    ...
    def _ensure_h264(self, stream: Go2RtcStream) -> str:
        ...
        with self._lock:
            streams = self._streams(stream.host, force=True)
    ...
    def _streams(self, host: str, force: bool = False) -> dict[str, Any]:
        ...
        with self._lock:
            self._streams_cache[host] = (now + self.cache_seconds, payload)
        return payload
