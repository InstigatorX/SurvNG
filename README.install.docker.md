...
SURVNG_IMAGE=ghcr.io/instigatorx/survng:v1.0
...
services:
  survng:
    image: ghcr.io/instigatorx/survng:v1.0-intel
    devices:
      - /dev/dri:/dev/dri
    environment:
      SURVNG_VIDEO_GID: ${SURVNG_VIDEO_GID:-44}
...
```bash
docker pull ghcr.io/instigatorx/survng:v1.0-model-installer
...
docker run --rm --user "${SURVNG_UID}:${SURVNG_GID}" \
  ...
  ghcr.io/instigatorx/survng:v1.0-model-installer \
  --device GPU
```
...
GPU) if you do not want the moving `v1.0` tip.
