...
export function TelemetryViewer({ data, cameraId, timeZone }) {
      ...
      {section === "detection" ? (
        <div className="detection-settings subsection-workspace">
          ...
          {detectionSection === "tracking" ? <section className="detection-settings-card wide-card">
            ...
            <details className="detection-compact-details">
              ...
              <div className="detection-field-grid advanced-tracking-grid">
                ...
                <label>Person ReID model<input value={config.detector?.tracking?.reid_model_path ?? ""} onChange={(event) => updateConfig(["detector", "tracking", "reid_model_path"], event.target.value)} placeholder="osnet_x1_0_msmt17.xml" /><small>OpenVINO whole-person embedding model. Prefer OSNet-x1.0 MSMT17 (`osnet_x1_0_msmt17.xml`) for People identity; Intel 0286 remains a fallback. Face-recognition models are not compatible.</small></label>
                <label>ReID preprocess<select value={config.detector?.tracking?.reid_preprocess ?? ""} onChange={(event) => updateConfig(["detector", "tracking", "reid_preprocess"], event.target.value)}>
                  <option value="">Auto from model name</option>
                  <option value="imagenet_rgb">ImageNet RGB (OSNet)</option>
                  <option value="raw_bgr">Raw BGR (Intel 0286)</option>
                </select><small>OSNet-x1.0 needs ImageNet RGB. Intel 0286 uses raw BGR. Auto picks from the model filename.</small></label>
          ...
          {detectionSection === "faces" ? <details className="detection-settings-card detection-feature-card wide-card" open>
            <summary><span className="detection-settings-card-icon"><ScanFace size={18} /></span><span><strong>People identity</strong><small>Fuse face recognition with whole-body appearance for named people.</small></span></summary>
            <div className="detection-feature-body detection-field-grid">
              ...
              <label>People to enrich<input type="number" min="1" max="16" step="1" value={config.detector?.face_enrich_max_people ?? 4} onChange={(event) => updateConfig(["detector", "face_enrich_max_people"], Number(event.target.value))} /><small>Dedicated face detection runs on this many person crops per frame.</small></label>
              <label>Unknown cluster threshold<input type="number" min="0.2" max="0.9" step="0.01" value={config.detector?.face_unknown_cluster_threshold ?? 0.55} onChange={(event) => updateConfig(["detector", "face_unknown_cluster_threshold"], Number(event.target.value))} /><small>How similar two unknown faces must be to share a cluster.</small></label>
              <div className="detection-settings-subhead"><strong>Suggestion thresholds</strong><small>Each modality can suggest a name. Body-only stays reviewable.</small></div>
              <label>Face Suggestion Threshold<input type="number" min="0" max="1" step="0.01" value={config.detector?.face_match_threshold ?? 0.3} onChange={(event) => updateConfig(["detector", "face_match_threshold"], Number(event.target.value))} /></label>
              <label>Body Suggestion Threshold<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_body_match_threshold ?? 0.7} onChange={(event) => updateConfig(["detector", "people_body_match_threshold"], Number(event.target.value))} /><small>Uses person ReID embeddings from tracking. Body-only matches stay reviewable.</small></label>
              <label>Fused Suggestion Threshold<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_fused_match_threshold ?? 0.45} onChange={(event) => updateConfig(["detector", "people_fused_match_threshold"], Number(event.target.value))} /><small>Used when face and body agree on the same person.</small></label>
              <label>Body reinforce threshold<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_body_reinforce_threshold ?? 0.75} onChange={(event) => updateConfig(["detector", "people_body_reinforce_threshold"], Number(event.target.value))} /><small>Minimum body score before fused auto-identify is allowed.</small></label>
              <label>Disagreement gap<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_modality_disagreement_gap ?? 0.12} onChange={(event) => updateConfig(["detector", "people_modality_disagreement_gap"], Number(event.target.value))} /><small>If face and body name different people and their scores are this close, SurvNG suggests no one.</small></label>
              <label>Face fusion weight<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_face_fusion_weight ?? 0.65} onChange={(event) => updateConfig(["detector", "people_face_fusion_weight"], Number(event.target.value))} /></label>
              <label>Body fusion weight<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_body_fusion_weight ?? 0.35} onChange={(event) => updateConfig(["detector", "people_body_fusion_weight"], Number(event.target.value))} /><small>Face and body weights should add to 1.0.</small></label>
              ...
              <label>References Per Person<input type="number" min="1" max="200" step="1" value={config.detector?.face_max_references ?? 20} onChange={(event) => updateConfig(["detector", "face_max_references"], Number(event.target.value))} /><small>SurvNG chooses clear face and body references; pinned references are always retained.</small></label>
              <label>Saved observation limit<input type="number" min="100" max="100000" step="100" value={config.detector?.face_max_observations ?? 1000} onChange={(event) => updateConfig(["detector", "face_max_observations"], Number(event.target.value))} /><small>Oldest observations are removed first.</small></label>
              <div className="detection-settings-subhead"><strong>Automatic identification</strong><small>Optional. Body-only never auto-identifies.</small></div>
              ...
              <label>Automatic Face Threshold<input type="number" min="0" max="1" step="0.01" value={config.detector?.face_auto_identify_threshold ?? 0.55} onChange={(event) => updateConfig(["detector", "face_auto_identify_threshold"], Number(event.target.value))} /></label>
              <label>Automatic Fused Threshold<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_fused_auto_threshold ?? 0.62} onChange={(event) => updateConfig(["detector", "people_fused_auto_threshold"], Number(event.target.value))} /><small>Requires face+body agreement.</small></label>
              <label>Face auto margin<input type="number" min="0" max="1" step="0.01" value={config.detector?.face_auto_identify_margin ?? 0.12} onChange={(event) => updateConfig(["detector", "face_auto_identify_margin"], Number(event.target.value))} /><small>Minimum lead over the next person for face-only auto-identify.</small></label>
              <label>Fused auto margin<input type="number" min="0" max="1" step="0.01" value={config.detector?.people_fused_auto_margin ?? 0.10} onChange={(event) => updateConfig(["detector", "people_fused_auto_margin"], Number(event.target.value))} /><small>Minimum lead over the next person for fused auto-identify.</small></label>
            </div>
          </details> : null}
          </div>
        </div>
