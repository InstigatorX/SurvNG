          ...
          {detectionSection === "tracking" ? <section className="detection-settings-card wide-card">
            ...
            <details className="detection-compact-details">
              ...
              <div className="detection-field-grid advanced-tracking-grid">
                ...
                <div className="detection-settings-subhead"><strong>SurvNG Hybrid tracking</strong><small>Production tracking uses SurvNG’s timestamp-aware geometry and selective appearance recovery. Sparse Identity, TrackTrack and BoT-SORT are available only through the incident Compare tool.</small></div>
                <label>Retention profile<select value={config.detector?.tracking?.tracking_profile ?? "default"} onChange={(event) => updateConfig(["detector", "tracking", "tracking_profile"], event.target.value)}><option value="default">Default</option><option value="person_retention">Person retention</option></select><small>Person retention raises lost-track grace floors and enables entity relink hints for offline Sparse Identity. Enable person ReID separately.</small></label>
                <label className="compact-toggle"><input type="checkbox" checked={config.detector?.tracking?.entity_relink_enabled ?? false} onChange={(event) => updateConfig(["detector", "tracking", "entity_relink_enabled"], event.target.checked)} /><span>Entity relink after long gaps</span><small>Allows Sparse Identity Compare runs to assign a durable entity ID to a new tracklet after ReID recovery without pretending the trajectory stayed continuous. Production Hybrid ignores this for association.</small></label>
