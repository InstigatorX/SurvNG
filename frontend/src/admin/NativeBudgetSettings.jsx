...
export default function NativeBudgetSettings({ values={}, defaults={}, overrides=false, grouped=false, onChange }) {
  ...
  const fields = (motion, keys = null) => <div className="form-grid">{BUDGET_FIELDS.filter(f=>Boolean(f.motion)===motion && (!keys || keys.includes(f.key))).map(f=><label key={f.key}>{f.label}<input aria-label={f.label} type="number" min={f.min} max={f.max} step={f.step}
  ...
  return <section className={grouped ? "detection-settings-card native-settings-card" : "sub-panel"}>
    ...
    {grouped ? <>
      <fieldset><legend>Detection rates</legend><p>Use fewer checks while quiet and more when activity approaches.</p>{fields(false, ['idle_fps', 'active_fps'])}</fieldset>
      <fieldset><legend>When to stay active</legend><p>Keep checking after activity and wake before objects enter an incident zone.</p>{fields(false, ['cooldown_seconds', 'approach_padding'])}</fieldset>
    </> : fields(false)}
    ...
    <details><summary>{grouped ? "Motion wake-up sensitivity" : "Native motion wake-up (gvamotiondetect)"}</summary>
      ...
      {grouped ? <>
        <fieldset><legend>Motion sensitivity</legend>{fields(true, ['motion_threshold', 'pixel_diff_threshold', 'min_rel_area'])}</fieldset>
        <fieldset><legend>Persistence</legend>{fields(true, ['min_persistence', 'max_miss', 'confirm_frames'])}</fieldset>
        <fieldset><legend>Motion matching</legend>{fields(true, ['block_size', 'iou_threshold', 'smooth_alpha'])}</fieldset>
      </> : fields(true)}
    </details>
  </section>;
}
