...
try {
  for (const width of [360, 390, 1440]) {
    ...
    await page.route("**/api/**", (route) => {
      ...
      if ((path.endsWith("/thumbnail.jpg") || path.endsWith("/preview.jpg"))) return route.fulfill({ status: imageFailure ? 404 : 200, contentType: "image/svg+xml", body: imageFailure ? "" : '<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="800"><rect width="1280" height="800" fill="#30483d"/><text x="440" y="400" fill="white" font-size="48">Incident image</text></svg>' });
    ...
    const frames = page.locator(".incident-detail-frame");
    assert.equal(await frames.count(), 4);
    await frames.nth(1).click();
    const viewer = page.getByRole("dialog", { name: "Evidence image viewer" });
    await viewer.waitFor();
    assert.match(await viewer.locator("img").getAttribute("src"), /events\/41\//);
    await viewer.getByRole("button", { name: "Next", exact: true }).click();
    assert.match(await viewer.locator("img").getAttribute("src"), /recordings\/preview/);
    await viewer.getByRole("button", { name: "Next", exact: true }).click();
    assert.match(await viewer.locator("img").getAttribute("src"), /events\/42\//);
    assert.equal(await viewer.getByRole("button", { name: "Next", exact: true }).isDisabled(), true);
