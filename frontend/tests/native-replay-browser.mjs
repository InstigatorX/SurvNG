...
const server = await createServer({ root:fileURLToPath(new URL("..",import.meta.url)), configFile:false, cacheDir:join(temporary,"vite-cache"), optimizeDeps:{include:["shaka-player"]}, server:{host:"127.0.0.1",port:0}, plugins:[{
  ...
  load(id){if(id==="/entry.jsx")return `import React from 'react';import {createRoot} from 'react-dom/client';import {IncidentClipLayer} from '/src/incidents/IncidentCard.jsx';import {EventOverlay} from '/src/shared/evidence.jsx';const fullEvent=${JSON.stringify(event)};function growingEvent(end){const item=structuredClone(fullEvent);item.object_tracking.state='active';item.object_tracking.tracks[0].box_history[1][0]=${epoch}+end;item.object_tracking.tracks[0].trajectory[1][0]=${epoch}+end;return item;}function App(){const [event,setEvent]=React.useState(location.search.includes('growing')?growingEvent(8):location.search.includes('late')?{...fullEvent,object_tracking:{...fullEvent.object_tracking,tracks:[]}}:fullEvent);const [open,setOpen]=React.useState(true);window.completeTracks=()=>setEvent({...fullEvent,object_tracking:{...fullEvent.object_tracking,state:"complete"}});window.growTracks=()=>setEvent(growingEvent(15));window.confirmGeometry=()=>setEvent({...fullEvent,object_tracking:{...fullEvent.object_tracking,recording_overlay_compatible:true}});window.reopen=()=>setOpen(value=>!value);return open?(location.search.includes('modal')?<EventOverlay event={event} events={[event]} timeZone="UTC" onClose={()=>setOpen(false)} onSelect={()=>{}}/>:<IncidentClipLayer event={{...event,object_tracking:undefined}} trackingEvent={event} active={true} analysisMode="tracks"/>):null;}createRoot(document.getElementById('root')).render(<App/>);`;}
}]});
let browser;
try{
