...
assert.match(recordingsSource, /const \[aiOverlayEnabled, setAiOverlayEnabled\] = useState\(false\)/);
assert.match(recordingsSource, /const aiOverlayActive = aiOverlayEnabled/);
assert.match(recordingsSource, /&& heroPlaying/);
assert.match(recordingsSource, /&& !heroSeeking/);
assert.match(recordingsSource, /!Number\.isFinite\(frameSearchEpoch\)/);
assert.match(recordingsSource, /!Number\.isFinite\(pendingSeekEpochRef\.current\)/);
assert.match(recordingsSource, /<DebugDetectionOverlay/);
assert.match(recordingsSource, /key=\{aiOverlayKey\}/);
assert.match(recordingsSource, /aria-pressed=\{aiOverlayEnabled\}/);
assert.match(recordingsSource, /<Sparkles size=\{15\} \/>AI/);
