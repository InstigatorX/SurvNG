...
function summarizeFrames() {
  ...
  for (let index = 0; index < frames.length; index += 1) {
    ...
    if (previous && !previous.paused && !frame.paused && previous.playRun === frame.playRun && frame.time > previous.time && frame.time - previous.time < 1) {
...
function Fixture() {
  ...
  useEffect(() => {
    ...
    const sample = () => {
      ...
      if (video.readyState >= 2 && video.videoWidth) {
        ...
        frames.push({ time: video.currentTime, wallTime: performance.now(), paused: video.paused, playRun,
          rgb: [...ctx.getImageData(0, 0, 1, 1).data].slice(0, 3), source: currentSource.current });
      }
      if (video.requestVideoFrameCallback) frameHandle = video.requestVideoFrameCallback(sample);
    };
    if (video.requestVideoFrameCallback) frameHandle = video.requestVideoFrameCallback(sample);
    const timer = window.setInterval(() => setStatus(snapshot()), 150);
    return () => { running = false; window.clearInterval(timer); if (frameHandle) video.cancelVideoFrameCallback?.(frameHandle); };
  }, []);
  return <main style={{ maxWidth: 840, margin: "24px auto", padding: 16, fontFamily: "system-ui", color: "#17212b" }}>
    ...
    <p role="status">{status.transport} · {status.paused ? "Paused" : "Playing"} · {Number(status.time || 0).toFixed(2)} / {status.duration?.toFixed(2) || "—"} seconds · {status.rate || 1}× · Metadata {status.metadata || 0} · Ready {status.ready || 0} · Decoded frames {status.frames || 0}</p>
    <p data-testid="frame-summary">Decoded colors: {status.frameSummary?.colors.join(" → ") || "—"} · Black frames: {status.frameSummary?.blackFrames || 0} · Largest playing-frame interval: {status.frameSummary?.maxPlayingIntervalMs || 0} ms</p>
