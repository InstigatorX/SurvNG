...
assert.equal(relatedEvidenceLabel({ sequence_delta_seconds: 4.2 }), "Likely sequence · 4s");
assert.equal(relatedEvidenceLabel({ relation_type: "expected_route", sequence_delta_seconds: 4.2 }), "Expected route · 4s");
assert.equal(relatedEvidenceLabel({ relation_type: "appearance_route", sequence_delta_seconds: 4.2, similarity: 0.834, visually_similar: true }), "Expected route · Appearance 83% · 4s");
