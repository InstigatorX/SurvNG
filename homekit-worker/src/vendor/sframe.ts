// MIT, copyright seydx; provenance and modifications: ./README.md.
import { createCipheriv, createDecipheriv, createHmac, hkdfSync, randomBytes, timingSafeEqual } from 'node:crypto';

import type { SFrameKeyData } from '@homebridge/hap-nodejs';
...
export function encodeHeader(kid: bigint, ctr: bigint): Buffer {
  if (kid < 0n || ctr < 0n || kid > 0xffffffffffffffffn || ctr > 0xffffffffffffffffn) {
    throw new Error('SFrame header integer out of range');
  }
  let config = 0;
  const extra: Buffer[] = [];
...
function open(keys: DerivedKeys, suite: SFrameCipherSuite, buffer: Buffer): Buffer {
  ...
  if (body.length < params.nt) throw new Error('truncated SFrame tag');
...
function decodeHeader(buffer: Buffer): { header: Buffer; kid: bigint; ctr: bigint; offset: number } {
  if (!buffer.length) throw new Error('missing SFrame header');
  const config = buffer[0];
  const required = 1 + ((config & 0x80) ? ((config >> 4) & 7) + 1 : 0) + ((config & 8) ? (config & 7) + 1 : 0);
  if (buffer.length < required) throw new Error('truncated SFrame header');
  let offset = 1;
