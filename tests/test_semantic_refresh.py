...
def test_cover_refresh_supersedes_inflight_and_queued_evidence(tmp_path):
    ...
    assert service._queue.empty()  # Same-revision submissions coalesce.
...
def test_refresh_retains_vectors_but_hides_stale_evidence_until_replacement(tmp_path):
    service, event, change_cover = _revision_service(tmp_path)
    assert service.index_event(event) == 2
    change_cover()
    ...
    with service.index._connect() as connection:
        rows = connection.execute("select image_path from semantic_embeddings").fetchall()
    assert len(rows) == 2 and {row[0] for row in rows} == {"old.png"}
