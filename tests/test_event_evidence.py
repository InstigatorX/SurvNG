...
def test_cover_promotion_preserves_source_geometry_and_clears_stale_mask(tmp_path):
    store = EventStore(tmp_path)
    ...
    with store._connect() as conn:
        source = conn.execute("select observation_json from event_source_observations where event_id=? and snapshot_path='live.webp'",
                              (event["id"],)).fetchone()
    assert json.loads(source[0]) == original
    assert updated["evidence_revision"] == 2
...
def test_attempt_diagnostics_are_bounded_and_face_terminal_state_is_explicit(tmp_path):
    store = EventStore(tmp_path)
    ...
    assert store.get(event["id"])["evidence_revision"] == 1
    assert store.settle_face_evidence(event["id"], state="unavailable", reason="no_eligible_face")
    assert not store.settle_face_evidence(event["id"], state="unavailable", reason="no_eligible_face")
    assert all(item.get("status") != "face_evidence_pending" for item in json.loads(store.get(event["id"])["objects_json"]))
    assert store.get(event["id"])["evidence_revision"] == 1
    assert store.pending_evidence_updates()[-1]["kind"] == "incident_metadata_updated"
...
def test_tracking_progress_does_not_churn_visual_revision_or_source_archive(tmp_path):
    store = EventStore(tmp_path)
    event = add(store)
    source_count = len(store.source_observations(event["id"]))
    for observation_count in range(20):
        store.update_object_tracking(event["id"], {"state": "active", "observations": observation_count})
    assert store.get(event["id"])["evidence_revision"] == 1
    assert len(store.source_observations(event["id"])) == source_count
...
def test_metadata_outbox_ack_cannot_erase_concurrent_same_revision_state(tmp_path):
    ...
    store.settle_face_evidence(event["id"], state="unavailable", reason="none")
...
def test_buffered_cover_retains_source_reference_without_claiming_exact_utc(tmp_path):
    store = EventStore(tmp_path)
    event = add(store)
    reference = {"source": "buffered_main", "session": "capture-a", "generation": 3,
                 "ordinal": 12, "pts_ns": 400000000, "source_timestamp_exact": True,
                 "utc_timestamp_exact": False}
    candidate = {"label": "person", "temporal_consensus": True,
                 "box": {"x1": 300, "y1": 300, "x2": 500, "y2": 800},
                 "frame_reference": reference, "frame_timestamp_uncertainty_seconds": .125}
    updated = promote(store, event["id"], source="buffered_main", timestamp_exact=True,
                       cover_objects=[candidate])
    person = json.loads(updated["objects_json"])[0]
    assert person["snapshot_frame_reference"] == reference
    assert person["snapshot_timestamp_uncertainty_seconds"] == .125
    assert person["snapshot_timestamp_exact"] is False
    assert store.cover_requirement(event["id"])["state"] == "satisfied"
