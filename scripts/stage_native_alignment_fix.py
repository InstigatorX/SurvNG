...
anchor = '''def shortlist(candidates, limit=3):
    selected = []
...
'''
replacement = '''def shortlist(candidates, limit=3):
    selected = []
    for candidate in sorted(candidates, key=lambda c: c.score, reverse=True):
        if all(abs(candidate.epoch - previous.epoch) >= 1 for previous in selected):
            selected.append(candidate)
...
calibration_method = '''    def replay_calibration_observations(self, event, tracking):
...
new_process = '''    def _process(self, event_id, candidates, assets):
...
old_test = '''def test_optional_calibration_failure_does_not_block_disabled_mode_promotion(tmp_path):
...
'''
new_test = '''def test_optional_calibration_failure_keeps_substream_when_timing_unverified(tmp_path):
    from survng.app.config import CameraConfig
    service, events, event, image, obj, tracking = fixture(tmp_path)
    service.config.detector.native.verification_enabled = False
    service.config.cameras = [CameraConfig(id='test', name='Test', stream_url='rtsp://unused.invalid', native_same_field_of_view=True)]
